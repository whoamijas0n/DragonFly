 ...
 * 
 * Modificaciones:
 * - Añadido comando CMD:SEND para transmitir tramas HEX en 433 MHz.
 * - Nuevo estado SENDING_PACKET y lógica de transmisión con el módulo TX.
 */
...
// Variables para el comando SEND
volatile String txDataHex = "";
...
void TaskRF(void *pvParameters) {
  ...
  for (;;) {
      ...
      switch (currentState) {
        ...
        case SENDING_PACKET: {
          ...
          if (len > 0) {
            ...
            radioTX.startTransmit(bytes, len);
            delete[] bytes;
          }
          txDataHex = ""; // Limpiar para evitar reenvíos
          break;
        }
    ...
    switch (currentState) {
      ...
      case SENDING_PACKET:
        // Cuando la transmisión finalice, volver a IDLE
        if (radioTX.finished()) {
