...
# BASE_DIR apunta a la carpeta donde vive raspi.py, funcione
# el script como .py suelto o empaquetado (ej. PyInstaller).
BASE_DIR = Path(getattr(sys, "_MEIPASS", Path(__file__).resolve().parent))

app_api = FastAPI(title="DragonFly Remote")

# Monta /static -> sirve CSS/JS
app_api.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="static")
...
'''
    ...
    def _activar_modo_remoto(self):
        ...
        # Generar el código QR de manera segura
        try:
            ...
            lbl_qr = tk.Label(scroll.scrollable_frame, image=self.photo_qr, bg="#0c0c0c")
            lbl_qr.pack(pady=2)
            
            ttk.Label(scroll.scrollable_frame, text="Conectar a: DragonFly_UI\nNavegador: 192.168.4.1:8000", style='Gray.TLabel', justify='center').pack(pady=4)
        except Exception as e:
            self.escribir_consola(f"[!] Error procesando QR: {e}")
